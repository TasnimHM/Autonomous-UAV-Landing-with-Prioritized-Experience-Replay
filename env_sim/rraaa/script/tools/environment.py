...
class Environment():
    ...
    def __init__(self, args, client, config):
        ...
        self.sub_jax_guam_pose = rospy.Subscriber(
            f'/{self.vehicle_type}/pose', PoseStamped, self.callback_jax_guam_pose
        )
    ...
    def start(self):
        ...
        self.camera_front = SensorManager(
            self.world, 'RGBCamera',
        ...
        self.camera_left = SensorManager(
            self.world, 'RGBCamera',
            carla.Transform(carla.Location(x=0, z=2.4), carla.Rotation(yaw=-90)),
            self.ego_vehicle, {'fov': '90.0', 'image_size_x': '400', 'image_size_y': '400'}
        )
        self.camera_right = SensorManager(
            self.world, 'RGBCamera',
            carla.Transform(carla.Location(x=0, z=2.4), carla.Rotation(yaw=+90)),
            self.ego_vehicle, {'fov': '90.0', 'image_size_x': '400', 'image_size_y': '400'}
        )
        self.camera_back = SensorManager(
            self.world, 'RGBCamera',
        ...
        self.transform_lidar_from_vehicle = carla.Transform(
            carla.Location(x=0, y=0, z=0), carla.Rotation(yaw=+00, roll=00)
        )
        self.lidar_sensor = SensorManager(
            self.world, 'LiDAR',
            self.transform_lidar_from_vehicle,
            self.ego_vehicle, {
                'channels':            '64',
                'range':               '100',
    ...
    def reset(self, pose=None):
        """
        Teleport the ego vehicle to a new starting position for the next episode.

        Parameters
        ----------
        pose : geometry_msgs/Pose or None
            If supplied (sent by episode_manager via /episode/initial_pose),
            the vehicle is teleported to that position.
            If None, falls back to the static YAML config location.

        Why the old code was broken
        ---------------------------
        The original reset() did:
            spawn_point = random.choice(self.world.get_map().get_spawn_points())
            self.ego_vehicle.set_transform(spawn_point)

        Two problems with that:
        1. get_spawn_points() returns ground-level road intersections in Town02,
           nowhere near the helipad at altitude.
        2. callback_jax_guam_pose() runs every tick and calls set_transform()
           with whatever GUAM reports — so the teleport was overwritten within
           one tick, dragging the actor straight back to wherever GUAM thought
           it was (the old landing position). That is why you saw the brief
           ascent followed immediately by descent on every "reset".

        The fix works because node_vehicle.py resets b_state at the same time,
        so the very next GUAM publish comes from the new position — both sides
        now agree.
        """
        print('RESET CALLED!')

        if pose is not None:
            # Randomized position from episode_manager
            ...
            log.info(
                f"[Environment] Reset with pose → "
                f"x={pose.position.x:.1f}  y={pose.position.y:.1f}  z={pose.position.z:.1f}"
            )
        else:
            # Fall back to static YAML config location
            cfg_loc = self.config['ego_vehicle']['location']
            new_location = carla.Location(
                x=cfg_loc['x'],
                y=cfg_loc['y'],
                z=cfg_loc['z'],
            ...
            log.warning(
                "[Environment] Reset called with no pose — using YAML default location. "
                "Did episode_manager publish /episode/initial_pose before reset_called?"
            )
        ...
        self.world.tick()  # flush the transform before next GUAM pose arrives
        log.info(
            f"[Environment] ✅ Teleported to "
            f"x={new_location.x:.1f}  y={new_location.y:.1f}  z={new_location.z:.1f}"
        )
        return 0
    ...
    def publish_states(self):
        ...
        self.pub_world_state.publish(
            pack_multiarray_ros_msg(self.msg_mat.mat, world_state, label_row, label_col)
        )
        ...
        multiarray_ros_msg = pack_multiarray_ros_msg(
            self.msg_mat.mat, state_val_np,
            ','.join(veh_list_str), ','.join(state_key)
        )
    ...
    def set_weather_from_config(self):
        try:
            ...
            if weather_flag == 'custom':
                ...
                raise ValueError(f"Invalid weather_flag '{weather_flag}'.")
            ...
            if lighting_flag == 'custom':
                ...
                raise ValueError(f"Invalid lighting_flag '{lighting_flag}'.")
