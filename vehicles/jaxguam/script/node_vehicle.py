...
Fix applied 2026-04-29:
  • guam_velocity_cmd_callback was using dt=0.005 (GUAM internal timestep)
    for the position update, but the callback only fires at 10Hz (dt=0.1s).
    This caused a 20x attenuation of the descent velocity — UAV only moved
    0.1 m/s despite being commanded 0.79 m/s. Fixed by using dt=0.1.
"""
...
class GUAM_Node(Vehicle_Node):
    ...
    def guam_velocity_cmd_callback(self, msg: Twist):
        ...
        # ── FIX: use controller publish rate (10Hz = 0.1s) not GUAM internal ──
        # The callback fires at 10Hz so each call represents 0.1s of motion.
        # Using GUAM's internal dt=0.005 was causing 20x velocity attenuation,
        # resulting in only ~0.1 m/s actual descent despite commanding ~0.8 m/s.
        dt = 0.1   # was: getattr(self.guam, "dt", 0.005)
